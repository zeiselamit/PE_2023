...
cellid_type = loadCellFile('vascular_step2_cellid_cluster_beforeclean_13-Oct-2021.txt');
typenum = cell2mat(cellid_type(:,2));
clean_list = loadCellFile('cleaning_vascular_final_211013.txt');
...
cellid_type(typenum==0,:) = [];% selecting only vascular
...
linkaxes([ax1,ax2],'y')
if savefig_flag==1
    savefig(gcf,['tree_step3_vascular_',date,'.fig'])    
end
...
if savefig_flag==1
    savefig(gcf,['tsne_by_cluster_step3_vascular_perplexity_',num2str(perplexity),'_',date,'.fig'])
    eval(['export_fig tsne_by_cluster_step3_vascular','_',date,'.pdf']);
end
%% tsne by sample
...
if savefig_flag==1
    savefig(gcf,['vascular_step3_tsne_by_sample_perplexity_',num2str(perplexity),'_',date,'.fig'])
...
if savefig_flag==1
    savefig(gcf,['vascular_step3_median_per_condition_',date,'.fig'])
    eval(['export_fig vascular_step3_median_per_condition_',date,'.pdf']);
end
%% marker table 
...
if savefig_flag==1
    savefig(gcf,['vascular_step3_markertable_',date,'.fig']) 
    eval(['export_fig vascular_step3_markertable_',date,'.pdf']);
end


% % % % % % % % % % % % % % % % % % % % % % % % % % 
table2 = [cellid_sorted,m2c(T_cells_tmp)];
saveCellFile(table2,['vascular_step3_cellid_cluster_beforeclean_',date,'.txt']);
...
% % % % % % % % % % % % % % % % % % % % % % 
list = {'CD34','ACTA2','FBLN2','ISG15','MX1','NOTUM','ADM','HLA-G','DIO2','TCF21','CXCL14','XAGE2','EGFR','XAGE3','CSH2'};;
figure;
set(gcf,'color','w','position',[20,20,1100,960])
[ha, pos] = tight_subplot(4, 4, [0.02,0.02], [0.02,0.02], [0.02,0.02]);
for i=1:length(list)
