...
cellidclsuter_tb = loadCellFile('vascular_step3_cellid_cluster_beforeclean_17-Oct-2021.txt');


tropho_clustername = loadCellFile('cluster_name_vascular_221120.txt');
tropho_clustername(:,2) = cellfun(@(x) ['VASCULAR_',x], tropho_clustername(:,2),'UniformOutput',0);
tropho_clustername(:,2) = regexprep(tropho_clustername(:,2),'VASCULAR_EXC','EXC');
...
% saveCellFile(table1,['gene_correlation_step4_vascular_',date,'.txt'])
% % % % % % % % % % % 
...
% T_cells_tmp(523:574) = 1; % adjusting 1st cluster
...
if savefig_flag==1
    writecell(cellid_tsne,['Vascular_cellid-tsne_data_',date,'.csv']);
end
%% tsne by cluster
figure;
set(gcf,'color','w','position',[20,20,900,800])
...
if savefig_flag==1
    savefig(gcf,['tsne_final_step4_vascular_by_cluster_',date,'.fig'])    
end
if savefig_pdf==1
    eval(['export_fig tsne_final_step4_vascular_by_cluster_',date,'.pdf']);
end
%% flag sorting 
...
h1 = axes('position',[0.03,0.1,0.2,0.8]);
...
h2 = axes('position',[0.3,0.1,0.7,0.8]);
...
linkaxes([h1,h2],'y');
if savefig_flag==1
    savefig(gcf,['tree_step4_vascular_',date,'.fig'])    
    eval(['export_fig tree_step4_vascular_',date,'.pdf']);
end
...
if savefig_flag==1
    savefig(gcf,['vascular_step4_tsne_by_sample_perplexity_',num2str(perplexity),'_',date,'.fig'])
...
saveCellFile(table1,['FC_final_step4_vascular_dendrogram_junction_split_markers_',date,'.txt']);
saveCellFile(table2,['FC_final_step4_vascular_dendrogram_junction_split_markers_by_average_',date,'.txt']);
...
for j=1:length(T_cells_tmp_uni)
    ...
    eval(['export_fig vascular_abundance_celltype_221120/',clusteruni{j},'_sample_celltype_abundance_per_condition_',date,'.pdf']);
    close(hf2)
end
%donor
for j=1:length(T_cells_tmp_uni)
    ...
    ylabel('diff %');
    set(ax2,'ylim',[-200,200],'xlim',[0.5,2.5],'xtick',[1:2],'xticklabel',{'pe/ce','pl/ct'},'ytick',[-200,0,200],'fontname','arial','fontsize',12)
    eval(['export_fig vascular_abundance_celltype_221120/',clusteruni{j},'_donor_celltype_abundance_per_condition_',date,'.pdf']);
    close(hf2)
end
...
%     savefig(gcf,['immune_step4_median_per_condition_',date,'.fig'])
    eval(['export_fig vascular_step4_stackedbar_celltype_per_sample_',date,'.pdf']);
end
%% markertable
...
if savefig_flag==1
    savefig(gcf,['vascular_step4_markertable_',date,'.fig']) 
    eval(['export_fig vascular_step4_markertable_',date,'.pdf']);
end
...
gr2 = find((T_cells_tmp ~= 3));
% in = find(mean(data_orig_all_sorted(:,gr1)>0,2)>0.03 | mean(data_orig_all_sorted(:,gr2)>0,2)<0.7);
in = find(mean(data_orig_all_sorted>0,2)>0.05 & mean(data_orig_all_sorted>0,2)<0.99 & ~ismember(geneid_all(:,1),sex_genes));
...
ylabel(['-log10(q)']);
grid on;
title(['comparing ',gr1name,' to ',gr2name,' in cluster ',num2str(c1),', vascular'])
[~,xi] = sort(d);
axes(ha(2))
plot(x1, x2, '.');hold on;
xmax = max(x1);
...
ylabel(gr2name)
axis tight
% eval(['export_fig scatter_volcano_',gr1name,'_',gr2name,'_vascular_cluster',num2str(c1),'_',date,'.pdf']);
% % % % % % % 
 % 
...
%% print gene in tsne
list = {'MCAM','PDGFRB','ACTA2','DES','CSPG4','NES','RGS5'};
figure;
set(gcf,'color','w','position',[20,20,1100,960])
[ha, pos] = tight_subplot(2, 4, [0.02,0.02], [0.02,0.02], [0.02,0.02]);
for i=1:length(list)
    genePlot = list{i};
    markergene = (data_orig_all_sorted(strcmpi(geneid_all,genePlot),:));
    if length(markergene(:,1))>1
        markergene = markergene(1,:);
    end
    inpos = markergene>0;
...
list = unique({'PTPRB','ANGPT2','LRRC36','RSAD2',...
    'TOP2A','GJA5','ACKR1','PDGFRB','MX2','P2RY14','ACTA2',...
    'ADM','IGFBP5','RSAD2','CPE','TOP2A','GJA5','SBSPON','MX2','AREG','CD34'});
% list = {'mx1'};
typeplot = T_cells_tmp_uni;
fnorm = normpdf(norminv([0.001:0.01:0.999]));
for kk = 1:length(list)
    hf = figure('color','w','position',[20,20,520,55],'name',list{kk});
    ...
    save2png(['vascular_marker_violin/',list{kk},'_MarkersViolin_Vascular','_',num2str(round(p99)),'_',date],gcf,1000)
    close(hf)
end