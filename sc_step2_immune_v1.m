...
cellid_type(~(typenum==1 | typenum==3),:) = [];% selecting only immune 
...
if savefig_flag==1
    savefig(gcf,['tsne_by_cluster_step2_immune_perplexity',num2str(perplexity),'_',date,'.fig'])
    eval(['export_fig tsne_by_cluster_step2_immune','_',date,'.pdf']);
end
%% tsne by sample
...
if savefig_flag==1
    savefig(gcf,['immune_step2_tsne_by_sample_perplexity',num2str(perplexity),'_',date,'.fig'])
...
if savefig_flag==1
    savefig(gcf,['immune_step2_median_per_condition_',num2str(perplexity),'_',date,'.fig'])
    eval(['export_fig immune_step2_median_per_condition_','_',date,'.pdf']);
end
%% marker table
...
if savefig_flag==1
    savefig(gcf,['immune_step2_markertable_',date,'.fig']) 
    eval(['export_fig immune_step2_markertable_','_',date,'.pdf']);
end

% % % % % % % % % % % % % % % % % % % % % % % % % % 
table2 = [cellid_sorted,m2c(T_cells_tmp)];
saveCellFile(table2,['immune_step2_cellid_cluster_beforeclean_',date,'.txt']);
