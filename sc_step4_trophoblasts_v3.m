...
tropho_clustername = loadCellFile('cluster_name_trophoblast_221120.txt');
...
% saveCellFile(table1,['gene_correlation_step4_TB_',date,'.txt'])
% % % % % % % % % % % 
...
%     m_ratio(m_ratio<1) = -1./m_ratio(m_ratio<1);
    ...
    eval(['export_fig TB_abundance_celltype_221120/',clusteruni{j},'_sample_celltype_abundance_per_condition_',date,'.pdf']);
    close(hf2)
end
%donor
for j=1:length(T_cells_tmp_uni)
    ...
    ylabel('diff %');
    set(ax2,'ylim',[-200,200],'xlim',[0.5,2.5],'xtick',[1:2],'xticklabel',{'pe/ce','pl/ct'},'ytick',[-200,0,200],'fontname','arial','fontsize',12)
    eval(['export_fig TB_abundance_celltype_221120/',clusteruni{j},'_donor_celltype_abundance_per_condition_',date,'.pdf']);
    close(hf2)
end
...
% eval(['export_fig scatter_volcano_',gr1name,'_',gr2name,'_tb_cluster',num2str(c1),'_',date,'.pdf']);
%%
...
%% tsne marker genes
list = {'NCAM1','XIST','DDX3X','EIF1AX','DUSP9','FOXO4','XAGE3','SPINT1','PKIB','ACSS1'};
figure;
...
for kk = 1:length(list)
    hf = figure('color','w','position',[20,20,520*8/9,70],'name',list{kk});
