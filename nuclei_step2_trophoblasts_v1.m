...
cellid_type = loadCellFile('cellid_type_PE_QC800_400_30-May-2023.txt');
% typenum = cell2mat(cellid_type(:,2));
clustnum = cell2mat(cellid_type(:,3)); %by cluster
cellid_type(~(clustnum>=16 | (clustnum >= 9 & clustnum <= 11)),:) = [];% selecting only TB
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % %
%% top gene - scatter comparison
top_g = 50;
% gr1name = 'early control';
% gr2name = 'early PE';
gr1name = 'SYN';
gr2name = 'pre-fused';
gr1 = find( (T_cells_tmp >= 9 & T_cells_tmp ~= 19 ));
gr2 = find( (T_cells_tmp == 19 ));
...
for k=1:length(unique(T_cells_tmp(:,b)))
    ...
    axis tight
    yl = get(gca,'ylim');
    set(gca,'xtick',[1:4],'XTickLabel',[{'ce','pe'}],'yscale','linear','ylim',[-0.5,ceil(yl(2))+1])
    axis tight
