...
load PE_QC800_400_afterloading_12-Oct-2021.mat

cellid_type = loadCellFile('cellid_type_PE_QC800_400_12-Oct-2021.txt');
typenum = cell2mat(cellid_type(:,2));
cellid_type(~(typenum==4),:) = [];% selecting only stromal
...
donor_id_flag(strcmpi(sample,'68-2')) = repmat({'PE_late_2'},sum(strcmpi(sample,'68-2')),1);
% validcells = false(size(cellid));
% validcells(loc) = true;
sample_uni = {'48-1','53-1','61-1','63-1','63-2','67-1','67-2','68-1','68-2',...
    '77-1','78-1','86-1','87-1','88-1','92-1','99-1','101-1','103-1','104-1',...
    '105-1','106-1','107-1','107-2','108-1','113-2','114-2','116-1','117-1','119-1','120-1','120-2'};%
...
saveCellFile(table1,['gene_correlation_stromal_',date,'.txt'])
% % % % % % % % % % % 
...
linkaxes([ax1,ax2],'y')
if savefig_flag==1
    savefig(gcf,['tree_step2_stromal_',date,'.fig'])    
end
...
if savefig_flag==1
    savefig(gcf,['tsne_by_cluster_step2_stromal_perplexity',num2str(perplexity),'_',date,'.fig'])
    eval(['export_fig tsne_by_cluster_step2_stromal','_',date,'.pdf']);
end
...
if savefig_flag==1
    savefig(gcf,['stromal_step2_tsne_by_sample_perplexity',num2str(perplexity),'_',date,'.fig'])
...
condition_sorted(2,ctrl_flag_sorted) = 1;
condition_sorted(3,peearly_flag_sorted) = 1;
...
colorvec = distinguishable_colors(6);
...
for j=1:length(T_cells_tmp_uni)
    ...
    [~,p_ce_pe] = ttest2(t_cell{1},t_cell{3});
    box off
    axis tight
    yl = get(gca,'ylim');
    ...
    plot([1,3],yl(2)*0.9*[1,1],'k');
    text(2,yl(2)*0.9,num2str(p_ce_pe,2),'fontsize',5,'VerticalAlignment','bottom');
    set(ha(j),'xlim',[0.5,6.5],'xtick',[1:6],'xticklabel',{'ce','ct','pe','pl','iu+','iu-'})
    title(num2str(j))
end

if savefig_flag==1
    savefig(gcf,['stromal_step2_median_per_condition_',num2str(perplexity),'_',date,'.fig'])
    eval(['export_fig stromal_step2_median_per_condition_','_',date,'.pdf']);
end
%% marker table 
...
if savefig_flag==1
    savefig(gcf,['stromal_step2_markertable_',date,'.fig']) 
    eval(['export_fig stromal_step2_markertable_','_',date,'.pdf']);
end
...
saveCellFile(table2,['stromal_step2_cellid_cluster_beforeclean_',date,'.txt']);
...
nontype = zeros(4,length(T_cells_tmp_uni));
...
for jjj=1:length(T_cells_tmp_uni)
    ...
    nontype(:,jjj) = [max(tmp(1:3)),max(tmp(4:8)),max(tmp(9:11)),max(tmp(12:16))]';
    if sum(nontype(:,jjj))>1
        nontype_sc(T_cells_tmp==jjj) = 5;
    elseif sum(nontype(:,jjj))==0
        [~,imax] = max(mean(data_orig_all_sorted(loc,T_cells_tmp==jjj)>0,2));
        c = [1,1,1,2,2,2,2,2,3,3,3,4,4,4,4,4];
...
leg = {'immune','vascular','stromal','trophblast','doublets'};
legend(leg(unique(nontype_sc)))


% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
list = {'PAGE4','PEG10','PHLDA2','XAGE3','DSP','ITGA6','FBN2','NOTUM','PAPPA2','PSG3','PSG2','PSG4','PSG5','ERVFRD-1','PLAC8','LVRN'};
list = {'PTPRC','TYROBP','FCER1G','CLDN5','CD34','SPARCL1','HBA1','ACTA2','APOD','TCF21','CXCL14','XAGE2','EGFR','XAGE3','CSH2','HLA-G'};
figure;
...
list = {'PTPRC','NOTUM','DIO2','HLA-G','PRG2','LAIR2','TAC3','PSG1','PAPPA','PAPPA2','PRG2','ENG','CYP19A1','FLT1','RRAD'};
figure;
set(gcf,'color','w','position',[20,20,1100,960])
[ha, pos] = tight_subplot(4, 4, [0.02,0.02], [0.02,0.02], [0.02,0.02]);
for i=1:length(list)
...
% % % % % % % % % % % % % % % % % % % % % % 
list = {'EBI3','IGF2','ORMDL3','GATA2','KIR2DL4','NOTUM','PSG9','HLA-G','DIO2','TCF21','CXCL14','XAGE2','EGFR','XAGE3','CSH2'};;
figure;
set(gcf,'color','w','position',[20,20,1100,960])
[ha, pos] = tight_subplot(4, 4, [0.02,0.02], [0.02,0.02], [0.02,0.02]);
for i=1:length(list)
...
gr1name = 'pelate';
gr2name = 'peearly';
gr1 = find( (T_cells_tmp == 3 &(peearly_flag_sorted ==1)));
gr2 = find( (T_cells_tmp == 10 &(peearly_flag_sorted ==1)));
