...
cellid_type = loadCellFile('immune_step2_cellid_cluster_beforeclean_13-Oct-2021.txt');
typenum = cell2mat(cellid_type(:,2));
clean_list = loadCellFile('cleaning_immune_final_211013.txt');
...
linkaxes([ax1,ax2],'y')
if savefig_flag==1
    savefig(gcf,['tree_step3_immune_',date,'.fig'])    
end
...
if savefig_flag==1
    savefig(gcf,['tsne_by_cluster_step3_immune_perplexity_',num2str(perplexity),'_',date,'.fig'])
    eval(['export_fig tsne_by_cluster_step3_immune','_',date,'.pdf']);
end
%% tsne by sample
...
if savefig_flag==1
    savefig(gcf,['immune_step3_tsne_by_sample_perplexity_',num2str(perplexity),'_',date,'.fig'])
 
end

%% tsne by experinemtal group 
figure;
...
if savefig_flag==1
    savefig(gcf,['immune_step3_median_per_condition_',date,'.fig'])
    eval(['export_fig immune_step3_median_per_condition_',date,'.pdf']);
end
%% markertable 
...
if savefig_flag==1
    savefig(gcf,['immune_step3_markertable_',date,'.fig']) 
    eval(['export_fig immune_step3_markertable_',date,'.pdf']);
end
...
saveCellFile(table2,['immune_step3_cellid_cluster_beforeclean_',date,'.txt']);
...
list = {'LPL','CX3CR1','IFIT2','CFP','HES4','LAIR2','TAC3','PSG1','PAPPA','PAPPA2','PRG2','ENG','CYP19A1','FLT1','RRAD'};
figure;
set(gcf,'color','w','position',[20,20,1100,960])
[ha, pos] = tight_subplot(4, 4, [0.02,0.02], [0.02,0.02], [0.02,0.02]);
for i=1:length(list)
