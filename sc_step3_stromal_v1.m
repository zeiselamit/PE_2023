...
% cellid_type = loadCellFile('stromal_step2_cellid_cluster_beforeclean_13-Oct-2021.txt');
cellid_type = loadCellFile('stromal_step2b_cellid_cluster_beforeclean_08-Aug-2022.txt');
typenum = cell2mat(cellid_type(:,2));
% clean_list = loadCellFile('cleaning_stromal_final_211016.txt');
clean_list = loadCellFile('cleaning_stromal_step2b_220808.txt');
...
cellid_type(typenum==0,:) = [];% selecting only stromal
...
% % % % % % % % % % % 
m = mean(data(:,~female_flag)>0,2);
f = mean(data(:,female_flag)>0,2);
d = log2(m./f);
d(m<0.05 & f<0.05) = 0;
[~,xi] = sort(d);
figure;
plot(m,f,'.');hold on;
plot(m(xi(1:20)),f(xi(1:20)),'.r')
text(m(xi(1:20)),f(xi(1:20)),geneid(xi(1:20)))
xi = flipud(xi);
plot(m(xi(1:20)),f(xi(1:20)),'.r')
text(m(xi(1:20)),f(xi(1:20)),geneid(xi(1:20)))
...
linkaxes([ax1,ax2],'y')
if savefig_flag==1
    savefig(gcf,['tree_step3_stromal_',date,'.fig'])    
end
...
if savefig_flag==1
    savefig(gcf,['tsne_by_cluster_step3_stromal_perplexity_',num2str(perplexity),'_',date,'.fig'])
    eval(['export_fig tsne_by_cluster_step3_stromal','_',date,'.pdf']);
end
%% tsne by sample
...
if savefig_flag==1
    savefig(gcf,['stromal_step3_tsne_by_sample_perplexity_',num2str(perplexity),'_',date,'.fig'])
...
if savefig_flag==1
    savefig(gcf,['stromal_step3_median_per_condition_',date,'.fig'])
    eval(['export_fig stromal_step3_median_per_condition_',date,'.pdf']);
end
%% markertable 
...
if savefig_flag==1
    savefig(gcf,['stroaml_step3_markertable_',date,'.fig']) 
    eval(['export_fig stromal_step3_markertable_',date,'.pdf']);
end



%% removing TOP2A,MKI67- (minus) cycling cells
top2a = find(strcmpi(geneid,'TOP2A'));
mki67 = find(strcmpi(geneid,'MKI67'));
no_cyc = find(data_sorted_all(top2a,:)==0 & data_sorted_all(mki67,:)==0 & (T_cells_tmp==1 | T_cells_tmp==2)')';

%%
% % % % % % % % % % % % % % % % % % % % % % % % % % 
table2 = [cellid_sorted,m2c(T_cells_tmp)];
table2(no_cyc,2) = m2c(max(T_cells_tmp)+1);
saveCellFile(table2,['stromal_step3_cellid_cluster_beforeclean_',date,'.txt']);
