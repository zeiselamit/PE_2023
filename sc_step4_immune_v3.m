...
cellidclsuter_tb = loadCellFile('immune_step3_cellid_cluster_beforeclean_04-Aug-2022.txt');


tropho_clustername = loadCellFile('cluster_name_immune_221120.txt');
% tropho_clustername = loadCellFile('cluster_name_immune1.txt');
tropho_clustername(:,2) = cellfun(@(x) ['IMMUNE_',x], tropho_clustername(:,2),'UniformOutput',0);
tropho_clustername(:,2) = regexprep(tropho_clustername(:,2),'IMMUNE_EXC','EXC');
...
% %% nk cells split and bpro 22/08/04
% nk_pr = loadCellFile('nk_prss57_cellid_.txt');
% wnk = ismember(cellidclsuter_tb(:,1),nk_pr);
% cellidclsuter_tb(wnk,2) = {23};
% cellidclsuter_tb(wnk,3) = {'IMMUNE_NK_PRSS57'};
% bpro_hb = loadCellFile('bpro_lyve1_cellid.txt');
% whb = ismember(cellidclsuter_tb(:,1),bpro_hb);
% cellidclsuter_tb(whb,:) = [];
...
% saveCellFile(table1,['gene_correlation_step4_immune_',date,'.txt'])
% % % % % % % % % % % 
...
if savefig_flag==1
    writecell(cellid_tsne,['Immune_cellid-tsne_data_',date,'.csv']);
end
%% tsne by cluster
figure;
set(gcf,'color','w','position',[20,20,900,800])
...
if savefig_flag==1
    savefig(gcf,['tsne_final_step4_immune_by_cluster_',date,'.fig'])    
end
if savefig_pdf==1
    eval(['export_fig tsne_final_step4_immune_by_cluster_',date,'.pdf']);
end
% % % % % % % % % % % % 
colorsvec = distinguishable_colors(4);
% colors = distinguishable_colors(length(T_cells_tmp_uni)+1);
figure('color','w','position',[20,20,900,800]);
plot(mapped_xy(ctrlearly_flag,1),mapped_xy(ctrlearly_flag,2),'.','color',colorsvec(1,:),'markersize',4); hold on;
plot(mapped_xy(peearly_flag,1),mapped_xy(peearly_flag,2),'.','color',colorsvec(2,:),'markersize',4); hold on;
plot(mapped_xy(ctrl_flag,1),mapped_xy(ctrl_flag,2),'.','color',colorsvec(3,:),'markersize',4); hold on;
plot(mapped_xy(pelate_flag,1),mapped_xy(pelate_flag,2),'.','color',colorsvec(4,:),'markersize',4); hold on;
axis tight;
axis equal
axis off
xl = get(gca,'xlim');
...
lg=legend('ce','pe','ct','pl','Location','best');

set(ht,'fontsize',8,'FontName','arial');

colorsvec = distinguishable_colors(4);
% colors = distinguishable_colors(length(T_cells_tmp_uni)+1);
figure('color','w','position',[20,20,900,800]);
subplot(2,2,1)
dscatter(mapped_xy(ctrlearly_flag,1),mapped_xy(ctrlearly_flag,2)); hold on;
axis tight;
axis equal
axis off
subtitle('ce')
subplot(2,2,2)
dscatter(mapped_xy(peearly_flag,1),mapped_xy(peearly_flag,2)); hold on;
axis tight;
axis equal
axis off
subtitle('pe')
subplot(2,2,3)
dscatter(mapped_xy(ctrl_flag,1),mapped_xy(ctrl_flag,2)); hold on;
axis tight;
axis equal
axis off
subtitle('ct')
subplot(2,2,4)
dscatter(mapped_xy(pelate_flag,1),mapped_xy(pelate_flag,2)); hold on;
axis tight;
axis equal
axis off
subtitle('pl')
...
[bandwidth,density,X,Y] = kde2d(mapped_xy,n,xymin,xymax);
density_all = density;
...
colormap(cmap);
% scatter(mapped_xy(~gr1,1),mapped_xy(~gr1,2),8,0.8*[1,1,1],'filled'); hold on;
scatter(mapped_xy(gr1,1),mapped_xy(gr1,2),4,colorsvec(1,:),'filled'); hold on;
...
gr1 = peearly_flag;
% scatter(mapped_xy(~gr1,1),mapped_xy(~gr1,2),8,0.8*[1,1,1],'filled'); hold on;
scatter(mapped_xy(gr1,1),mapped_xy(gr1,2),4,colorsvec(2,:),'filled'); hold on;
axis tight, axis off, axis equal
title('PE')
axes(ha(3));
contourf(X,Y,density); hold on;
...
gr1 = ctrl_flag;
% scatter(mapped_xy(~gr1,1),mapped_xy(~gr1,2),8,0.8*[1,1,1],'filled'); hold on;
scatter(mapped_xy(gr1,1),mapped_xy(gr1,2),4,colorsvec(3,:),'filled'); hold on;
axis tight, axis off, axis equal
title('CT')
axes(ha(4));
contourf(X,Y,density); hold on;
...
gr1 = pelate_flag;
% scatter(mapped_xy(~gr1,1),mapped_xy(~gr1,2),8,0.8*[1,1,1],'filled'); hold on;
scatter(mapped_xy(gr1,1),mapped_xy(gr1,2),4,colorsvec(4,:),'filled'); hold on;
axis tight, axis off, axis equal
title('PL')

% if savefig_pdf==1
%     eval(['export_fig GABA_contours_female_male_',date,'.pdf']);
% end
figure;
set(gcf,'color','w','position',[20,20,1200,1200]);
[ha, pos] = tight_subplot(2, 2, [0.02,0.02], [0.02,0.02], [0.02,0.02]);
...
axes(ha(1))
gr1 = ctrlearly_flag;
[bandwidth,density_ce,X,Y] = kde2d(mapped_xy(gr1,:),n,xymin,xymax);
gr1 = peearly_flag;
[bandwidth,density_pe,X,Y] = kde2d(mapped_xy(gr1,:),n,xymin,xymax);
z = density_pe-density_ce; z(z<0) = 0;
...
axes(ha(3))
gr1 = ctrl_flag;
[bandwidth,density_ct,X,Y] = kde2d(mapped_xy(gr1,:),n,xymin,xymax);
gr1 = pelate_flag;
[bandwidth,density_pl,X,Y] = kde2d(mapped_xy(gr1,:),n,xymin,xymax);
z = density_pl-density_ct; z(z<0) = 0;
contourf(X,Y,z); hold on;
colormap(cmap);
axis tight, axis off, axis equal
title('PL-CT')
...
linkaxes([h1,h2],'y');
if savefig_flag==1
    savefig(gcf,['tree_step4_immune_',date,'.fig'])
    eval(['export_fig tree_step4_immune_',date,'.pdf']);
end
...
if savefig_flag==1
    savefig(gcf,['immune_step4_tsne_by_sample_perplexity_',num2str(perplexity),'_',date,'.fig'])
...
saveCellFile(table1,['FC_final_step4_immune_dendrogram_junction_split_markers_',date,'.txt']);
saveCellFile(table2,['FC_final_step4_immune_dendrogram_junction_split_markers_by_average_',date,'.txt']);
...
for j=1:length(T_cells_tmp_uni)
    ...
    eval(['export_fig immune_abundance_celltype_221120/',clusteruni{j},'_sample_celltype_abundance_per_condition_',date,'.pdf']);
    close(hf2)
end
%donor
for j=1:length(T_cells_tmp_uni)
    ...
    ylabel('diff %');
    set(ax2,'ylim',[-200,200],'xlim',[0.5,2.5],'xtick',[1:2],'xticklabel',{'pe/ce','pl/ct'},'ytick',[-200,0,200],'fontname','arial','fontsize',12)
    eval(['export_fig immune_abundance_celltype_221120/',clusteruni{j},'_donor_celltype_abundance_per_condition_',date,'.pdf']);
    close(hf2)
end
...
%     savefig(gcf,['immune_step4_median_per_condition_',date,'.fig'])
    eval(['export_fig immune_step4_stackedbar_celltype_per_sample_',date,'.pdf']);
end
...
if savefig_flag==1
    savefig(gcf,['immune_step4_markertable_',date,'.fig']) 
    eval(['export_fig immune_step4_markertable_',date,'.pdf']);
end
...
gr2name = {'fetus'};
c1 = 1;
% gr1 = find(T_cells_tmp >= 16 & T_cells_tmp <= 17 &(peearly_flag_sorted ==1));
% gr2 = find(T_cells_tmp >= 16 & T_cells_tmp <= 17 &(ctrl_flag_sorted ==1));
gr1 = find(T_cells_tmp == 12 & female_flag_sorted == 0 & xist'>0);
gr2 = find(T_cells_tmp == 12 & female_flag_sorted == 0 & xist'==0);
...
x1 = mean(log2(data_orig_all_sorted(in,gr1)+1),2);
x2 = mean(log2(data_orig_all_sorted(in,gr2)+1),2);
d = x1-x2 ;
figure('position',[200,200,1400,580],'color','w');
[ha, pos] = tight_subplot(1, 3, [0.05,0.05], [0.1,0.05], [0.05,0.05]);
axes(ha(1))
plot(d,-log10(ptt),'.'); hold on;
[~,xi] = sort(ptt);
plot(d(xi(1:100)),-log10(ptt(xi(1:100))),'or');
text(d(xi(1:100)),-log10(ptt(xi(1:100))),geneid_all(in(xi(1:100))));
xlabel([gr1name,'-',gr2name]);
ylabel(['-log10(q)']);
grid on;
title(['comparing ',gr1name,' to ',gr2name,' in cluster ',num2str(c1),', immune'])
[~,xi] = sort(d);
axes(ha(2))
plot(x1, x2, '.');hold on;
xmax = max(x1);
...
text(x1(xi(1:top_g)), x2(xi(1:top_g)),geneid_all(in(xi(1:top_g))),'fontsize',6);
xi = flipud(xi);
plot(x1(xi(1:top_g)), x2(xi(1:top_g)),'.r'); hold on;
text(x1(xi(1:top_g)), x2(xi(1:top_g)),geneid_all(in(xi(1:top_g))),'fontsize',6);
...
top_g = 50;
x1 = mean(data_orig_all_sorted(in,gr1)>0,2);
x2 = mean(data_orig_all_sorted(in,gr2)>0,2);
d = x1-x2 ;
[~,xi] = sort(d);
axes(ha(3))
plot(x1, x2, '.');hold on;
...
ylabel(gr2name)
axis tight
eval(['export_fig scatter_volcano_',gr1name,'_',gr2name,'_immune_cluster',num2str(c1),'_',date,'.pdf']);
%%
...
list = {'CD14','FCGR3A','FCGR1A','CD68','TFRC','CCR5'};
% list = {'SPINK2','SMIM24','CYTL1','PRSS57','CD34','HOPX','C1QTNF4','MLLT3','RPS3','CDCA7','RPS4X','NPM1','EEF1B2','APEX1','LDHB','ITM2A'};
figure;
...
% eval(['export_fig immune_step4_tsne_by_gene_','general-mac_',date,'.png']); 

%% markers violins
% list = {'CXCL9','APOE','IFIT3','EREG','FCN1','S100A12','FCGR3B','LYVE1','IFIT1','CD74','C1QA','TREM2'}; % myeloid
% list = {'PRSS57','KLRF1','XCL1','TOP2A','TNFRSF18','CD8B','CCR7','C1QTNF4','CD79A','GZMK','CD4'}; %lymphoid
list = {'CD34','PRSS57','IGLL1','SPINK2','C1QTNF4'};
fnorm = normpdf(norminv([0.001:0.01:0.999]));
typeplot = 10:18;
for kk = 1:length(list)
    hf = figure('color','w','position',[20,20,520,70],'name',list{kk});
...
%             fi = fi/max(fi);
    ...
    %     else
    set(gca,'xlim',[0,1+length(typeplot)],'xtick',[1:length(typeplot)],'xticklabel',cell(size(typeplot)),'ylim',[-1,p99+1],'YScale',yscale,'ytick',yt,'yticklabel',cell(length(yt),1))
    %     end
...
%     eval(['export_fig markersViolin_Immune_',char(list(kk)),'_',num2str(p99),'_',date,'.pdf']);
    save2png(['MarkersViolin_',list{kk},'_',num2str(round(p99)),'_',date],gcf,1000)
%     close(hf)
end
%% tsne - sex genes
list = {'XIST'};
figure;
set(gcf,'color','w','position',[20,20,500,600])
[ha, pos] = tight_subplot(1, 1, [0.02,0.02], [0.02,0.02], [0.02,0.02]);
for i=1:length(list)
    genePlot = list{i};
    markergene = (data_orig_all_sorted(strcmpi(geneid_all,genePlot),:));
%     markergene(female_flag_sorted')=0;
%     mapped_xy(~female_flag_sorted,:) = 0;
    inpos = markergene>0;
    tmpthlow = prctile(markergene(markergene>0),1);
    tmpthhigh = prctile(markergene(markergene>0),99);
    if tmpthlow==tmpthhigh
    ...
    markergene_color = [interp1([min(markergene),max(markergene)],[0.7,c_rgb(1)],markergene'),...
        ...
        ,interp1([min(markergene),max(markergene)],[0.7,c_rgb(3)],markergene')];
    axes(ha(i));
    scatter(mapped_xy(~(inpos & female_flag_sorted'),1),mapped_xy(~(inpos & female_flag_sorted'),2),20,0.6*[1,1,1],'.'); hold on; %change female_flag accordingly
    scatter(mapped_xy(inpos & female_flag_sorted',1),mapped_xy(inpos & female_flag_sorted', 2),20,[1,0,0],'.'); hold on; %change female_flag accordingly
    set(gca,'xlim',[-150,150],'ylim',[-150,150])
    title(genePlot);
    axis tight
    axis equal
    axis off
%     eval(['export_fig immune_step4_tsne_by_fetus_sex_','female_',char(list),'_',date,'.pdf']); % change sex accordingly
end